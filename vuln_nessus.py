...
def read_csv(csv_name, info_flag):
    ...
    data_pluginID = pd.read_csv(filepath_or_buffer=csv_name, engine='python', encoding='ISO-8859-1')["Plugin ID"].values
    data_name = pd.read_csv(filepath_or_buffer=csv_name, engine='python', encoding='ISO-8859-1')["Name"].values
...
def hebing(listHe):
    ...
    list_len = len(listHe)
    # 设置临时变量，作为缓冲输出
    tmp_pluginID, tmp_name, tmp_risk, tmp_host, tmp_solution = listHe[0][0], listHe[0][1], listHe[0][2], listHe[0][3], \
                                                               listHe[0][4]
    ...
    for x in range(list_len):
        ...
        # 输出漏洞详情和设置新的tmp_host
            ...
            tmp_host = listHe[x][3]
        # 设置这次的漏洞与下个漏洞做比较
        tmp_pluginID, tmp_name, tmp_risk, tmp_host, tmp_solution = listHe[x][0], listHe[x][1], listHe[x][2], tmp_host, \
                                                                   listHe[x][4]
    # #最后一个漏洞无法比较，要单独输出。
    return new_list
